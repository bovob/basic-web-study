...
@WebServlet(value="/ex04/s2", loadOnStartup=1)
...
public class Servlet02 extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        out.println("/ex04/s2 실행 | 보관소에 값을 뺐습니다.");
        out.printf("Context = %s\n", contextString);
        out.printf("Session = %s\n", sessionString);
        out.printf("Request = %s\n", reqString);


        // 지금 가지고 있는 값을 forward, include 한다.
        req.getRequestDispatcher("/ex04/s3").forward(req,resp);
        req.getRequestDispatcher("/ex04/s4").include(req,resp);
